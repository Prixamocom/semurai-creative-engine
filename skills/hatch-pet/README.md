...
checked into the Semurai Creative repo (rather than pulled in as a Git submodule
or an npm package) so that:

- Any Semurai Creative agent can run the skill end-to-end without a network
...
- Pinned upstream reference (declared in `SKILL.md` frontmatter): see the
  ...
  directory as the authoritative source-of-truth for Semurai Creative and
...
2. Replace the contents of this directory with the upstream snapshot,
   preserving only this `README.md` and any Semurai Creative-specific notes
   inside `SKILL.md`'s `> **Semurai Creative integration.**` blockquote.
...
flow (including how Semurai Creative behaves when Codex is not installed).
