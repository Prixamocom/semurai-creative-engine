...
Semurai Creative owns the V2 schemas, durable task-chain state, applied content
...
re-injected by Semurai Creative. Below, "form, freeze, or update" describes content
...
- The run state Semurai Creative records captures this Run's execution state and
...
Use the stage supplied by Semurai Creative:
...
9. Run Execution Preflight for every declared production route, dependency,
   ...
   Agent.
10. Emit a strict Plan Contract and Runtime State for Semurai Creative to parse.
...
dispatch deliverables. Semurai Creative starts Build by continuing the same native
...
Use the `contract_repair` stage only when Semurai Creative reports that the semantic
...
in context. Read it through the supplied Semurai Creative CLI wrapper; on POSIX
shells:
