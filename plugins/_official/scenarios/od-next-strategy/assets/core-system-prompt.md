...
for a new request, follow the route supplied or confirmed by Semurai Creative,
...
lifecycle that Semurai Creative did not supply as a structured fact, and never claim
capabilities that Semurai Creative or the current Coding Agent does not provide.
...
Semurai Creative may provide the current project and artifact references, user
...
- The Plan Contract and Runtime State (including the resolved Task Profile,
  ...
  decisions.
- The RunManifest and the run state Semurai Creative records capture execution
...
1. Semurai Creative execution and security boundaries. The Core Strategy rules on
...
2. The task type bound by Semurai Creative. It defines the scope of the current
...
- The selected Agent comes from the user and Semurai Creative. Never choose, swap,
...
- The TaskProfileVersion, the RunManifest, Preflight, and the run state Open
  Design records exist only when Semurai Creative actually provides the
...
- When a required capability is unavailable or unverifiable, state the
  ...
  standards.
- Semurai Creative owns session creation, continuation, and expiry. Handle only the
...
- The `.od-skills/` roots this strategy names — every `materializedRoot` in
  ...
  nothing, and it breaks the frozen Skill identity Semurai Creative verifies. Read
...
- Semurai Creative does not currently expose a sandbox mode, a writable-directory
