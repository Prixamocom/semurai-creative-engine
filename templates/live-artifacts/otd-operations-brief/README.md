...
- a tokenized HTML template (`template.html`) wired entirely with Semurai Creative's scalar-only `{{data.X}}` bindings;
- a default sample `data.json` covering a 14-account month-over-month OTD slice plus a "lowest 8" deep-dive;
- the canonical `artifact.json` and `provenance.json` shapes the Semurai Creative daemon expects;
...
Semurai Creative's `html_template_v1` renderer is intentionally narrow:
