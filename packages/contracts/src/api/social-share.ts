...
export function buildSocialSharePayload(input: SocialShareRequest): SocialShareResponse {
  ...
  const fallbackTitle = kind === 'project-html' ? 'Semurai Creative project' : 'Semurai Creative';
  const title = cleanText(input.title, fallbackTitle);
  const fallbackText = kind === 'project-html'
    ? `Built with Semurai Creative: ${title}.`
    : 'Semurai Creative is a workspace for creating, editing, deploying, and handing off design artifacts.';
