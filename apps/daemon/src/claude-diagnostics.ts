...
export function diagnoseClaudeCliFailure(
  ...
  if (authFailure) {
    ...
    const configHint = hasConfigDir
      ...
      : 'If you use multiple Claude profiles, set CLAUDE_CONFIG_DIR in Settings so Semurai Creative spawns the same profile that works in your terminal.';
    return withContext(
      'Claude Code could not authenticate. Run `claude`, use `/login`, then retry the Semurai Creative request.',
  ...
  if (windowsCredentialMismatch) {
    return withContext(
      ...
      'Re-authenticate Claude Code in the same Windows, WSL, or shell environment that Semurai Creative uses. On native Windows, check Windows Credential Manager if `/login` does not repair the session.',
      input,
    );
  }
  ...
  if (configStateFailure) {
    ...
    const detail = hasConfigDir
      ? 'Re-run `claude` and `/login` for that profile, then retry Semurai Creative.'
  ...
  if (!text.trim() && input.exitCode === 1) {
    ...
    const detail = hasConfigDir
      ? 'Re-run `claude` and `/login` for that profile, then retry Semurai Creative.'
      : 'Run `claude`, use `/login`, and retry. If you use multiple Claude profiles, set CLAUDE_CONFIG_DIR in Settings so Semurai Creative uses the same profile as your terminal.';
    return withContext(
      message,
      detail,
      input,
