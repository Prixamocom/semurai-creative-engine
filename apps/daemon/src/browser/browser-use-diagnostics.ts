...
export function isBrowserUseRequested(...values: unknown[]): boolean {
  return values.some((value) => (
    ...
    (
      ...
      value.includes('Use the selected Semurai Creative Browser tab as the bound target.')
    )
  ));
}
...
export function renderBrowserUseUnavailablePrompt(state: BrowserUseRunState | null): string {
  ...
  return [
    ...
    '',
    `Browser automation was requested, but Semurai Creative has not confirmed a matching in-app browser backend for this run. Reason: \`${state.reason}\`.`,
