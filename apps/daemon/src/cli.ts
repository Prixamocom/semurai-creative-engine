...
Options:
  ...
  --daemon-url <url>       Override the Semurai Creative daemon HTTP base.`);
}
...
Install or repair Semurai Creative's bundled connection component in the user's
official DeepSeek Harness installation. The dsh CLI itself is not installed
or upgraded by Semurai Creative.
...
function printRootHelp() {
  ...
  od plugin open-design-pr <folder>
      Push a community-catalog branch and open the Semurai Creative PR form.
  ...
  od amr <login|status> [args]
      ...
      the local Semurai Creative daemon.
  ...
  od share <open-design|url> [options]
      Build localized social-share targets for the Semurai Creative repo or a
  ...
  od mcp [--daemon-url <url>]
      ...
      running Semurai Creative daemon. Wire it into a coding agent
      (Claude Code, Cursor, VS Code, Zed, Windsurf) in another repo
      to pull files from a local Semurai Creative project and create
...
Options:
  --daemon-url <url>   Semurai Creative daemon HTTP base.
...
Runs Tavily-backed shallow research through the local Semurai Creative daemon.
  ...
  if (code === 'EPERM' || code === 'ENETUNREACH') {
    console.error(
      'hint: outbound connect was denied by a sandbox. If you launched ' +
        ...
        'network policy. The Semurai Creative daemon itself is unaffected - it can be ' +
...
Common options:
  ...
  --quality <tier>          Semurai Creative Cloud images only: published quality tier
  ...
  --resolution <res>        Semurai Creative Cloud images only: published output resolution
...
tool calls to a running Semurai Creative daemon. Wire it into a coding agent
in another repo so the agent can pull files from a local Semurai Creative
project and create project-scoped artifacts without exporting a zip
every iteration.

Options:
  --daemon-url <url>   Semurai Creative daemon HTTP base URL. Resolution
                       ...
                       survive a Semurai Creative restart.
...
Tools exposed:
  list_projects                  list every Semurai Creative project
...
user has open in Semurai Creative; get_artifact and get_file additionally
...
→ MCP server in the Semurai Creative app. The daemon must be running locally
...
Register Semurai Creative's stdio MCP server into a coding agent's own config.

Agents:
  ${AGENT_SLUGS.join(' ')}

Options:
  --uninstall, --remove   Remove the Semurai Creative MCP server instead.
...
Wraps GitHub CLI auth for Semurai Creative registry publishing. The token stays in gh.`);
    return;
  }
...
Shows the GitHub account gh will use for Semurai Creative registry publishing.`);
    return;
  }
  const host = typeof flags.host === 'string' ? flags.host : 'github.com';
...
Common options:
  --daemon-url <url>   Semurai Creative daemon HTTP base (default OD_DAEMON_URL, inherited sidecar discovery, or http://127.0.0.1:7456).
  ...
  switch (sub) {
    ...
    case 'login': {
      ...
      console.log(`[marketplace login] authenticating gh for ${host}. Tokens stay in gh, not Semurai Creative.`);
...
Imports a local project directory as an editable Semurai Creative design system.
...
Imports a public GitHub repository as an editable Semurai Creative design system.
...
Imports a shadcn registry item as a Semurai Creative design system.
...
async function runWhatsNew(args) {
  ...
  const data = await resp.json();
  if (flags.json) return process.stdout.write(JSON.stringify(data, null, 2) + '\n');
  console.log(`Semurai Creative ${data?.version ?? 'unknown'}`);
