...
export async function ensureMcpDaemonUrl(
  ...
  if (plan.action === "none") {
    ...
    throw new Error(
      `The registered Semurai Creative runtime is unavailable and cannot be launched (${plan.reason}).`,
    );
  }
  ...
  throw new Error(
    `Semurai Creative was launched headlessly but its daemon did not become ready within ${timeoutMs}ms.`,
  );
}
