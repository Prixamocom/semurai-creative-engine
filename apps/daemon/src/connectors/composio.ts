...
function fallbackComposioDescription(name: string, category: string | undefined): string {
  ...
  if (normalizedCategory?.includes('project')) return `Coordinate ${name} projects, tasks, and workflow data inside Semurai Creative.`;
  if (normalizedCategory?.includes('communication')) return `Bring ${name} conversations, channels, and collaboration context into Semurai Creative.`;
  if (normalizedCategory?.includes('documentation')) return `Search and reuse ${name} knowledge, pages, and documentation in Semurai Creative.`;
  if (normalizedCategory?.includes('storage')) return `Find and reference ${name} files, folders, and document metadata from Semurai Creative.`;
  if (normalizedCategory?.includes('developer')) return `Inspect ${name} developer resources, activity, and operational context from Semurai Creative.`;
  if (normalizedCategory?.includes('crm') || normalizedCategory?.includes('sales')) return `Use ${name} customer, deal, and account context in Semurai Creative artifacts.`;
  if (normalizedCategory?.includes('marketing')) return `Analyze ${name} campaigns, audiences, and marketing activity from Semurai Creative.`;
  if (normalizedCategory?.includes('finance') || normalizedCategory?.includes('commerce')) return `Work with ${name} business, billing, and transaction data in Semurai Creative.`;
  if (normalizedCategory?.includes('observability')) return `Surface ${name} incidents, metrics, and operational signals in Semurai Creative.`;
  if (normalizedCategory?.includes('data')) return `Query ${name} datasets and platform metadata for data-backed Semurai Creative artifacts.`;
  return `Use ${name} tools and data directly from Semurai Creative.`;
}
