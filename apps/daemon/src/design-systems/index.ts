...
export function buildDesignSystemSkillsMarkdown(input: {
  ...
  lines.push('');
  lines.push(
    'Generated with **Semurai Creative**. ' +
...
async function migrateLegacyDesignSystemPackage(
  ...
  const title = normalizeTitle(metadata.title ?? firstHeading(body) ?? id);
  const summary = summarize(body) || 'A reusable Semurai Creative design system.';
  ...
  await Promise.all([
    ...
    appKitExists
      ? writeIfMissing(
          ...
          `# ${title} UI Kit\n\nThis package was migrated from an earlier Semurai Creative design-system workspace. Use \`index.html\` as the applied interface example and replace it with source-backed modular components when new repository evidence is available.\n`,
        )
...
function generatedDesignSystemFileWrites(
  ...
  const summary = input.summary || 'A user-created Semurai Creative design system.';
...
function buildDraftDesignSystemBody(input: UserDesignSystemInput & { title: string }): string {
  ...
  const summary = cleanText(input.summary) || 'A user-authored design system for future Semurai Creative projects.';
...
A reusable Semurai Creative package for ${input.title}.
...
function renderSkill(input: {
  ...
  return `---
name: ${skillName}
description: Use this skill when generating Semurai Creative artifacts that should follow ${input.title}.
user-invocable: true
---
...
function renderOverviewHtml(
  ...
  return renderHtmlDocument(
    title,
    `<main class="overview">
      <p class="eyebrow">Semurai Creative system</p>
