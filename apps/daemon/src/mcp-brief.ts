...
const BRIEF_COPY: Record<LocalMcpBriefLocale, LocalizedBriefCopy> = {
  en: {
    ...
    description:
      'Choose one option for each decision. The same readable brief can be used with Semurai Creative Cloud or Local Codex.',
    submitLabel: 'Confirm brief',
    completeCard:
      'Complete the rendered Semurai Creative brief card. The confirmation returns a readable summary; internal correlation values must remain hidden.',
  ...
  'zh-CN': {
    ...
    description:
      '请为每项决策选择一个选项。确认后的可读需求可用于 Semurai Creative Cloud、本地 Codex 或安全 BYOK。',
    submitLabel: '确认需求',
    completeCard: '请填写显示的 Semurai Creative 需求卡。确认后会返回可读摘要；不要向用户展示内部关联值。',
    confirmed: '需求已确认。',
  ...
  'zh-TW': {
    ...
    description:
      '請為每項決策選擇一個選項。確認後的可讀需求可用於 Semurai Creative Cloud、本機 Codex 或安全 BYOK。',
    submitLabel: '確認需求',
    completeCard: '請填寫顯示的 Semurai Creative 需求卡。確認後會回傳可讀摘要；不要向使用者顯示內部關聯值。',
    confirmed: '需求已確認。',
  ...
  ja: {
    ...
    description:
      '各項目から 1 つ選択してください。確認したブリーフは Semurai Creative Cloud、ローカル Codex、安全な BYOK で利用できます。',
    submitLabel: 'ブリーフを確認',
    completeCard:
      '表示された Semurai Creative のブリーフカードに回答してください。確認後は読みやすい概要が返されます。内部の関連付け値は表示しないでください。',
...
export function createLocalMcpBriefStore(
  ...
  return {
    ...
    confirm(input) {
      ...
      if (!draft) {
        throw new Error(
          'The Semurai Creative brief has expired or is unknown. Call collect_brief again.',
        );
      }
      if (draft.nonce !== nonce) {
        throw new Error('The Semurai Creative brief nonce is invalid.');
      }
      ...
      if (!decision.complete) {
        ...
        throw new Error(
          `The Semurai Creative brief is incomplete. Missing: ${missing.join(', ')}.`,
        );
      }
      ...
      if (draft.confirmation) {
        if (draft.confirmationAnswersDigest !== confirmationAnswersDigest) {
          throw new Error(
            'This Semurai Creative brief was already confirmed with different answers.',
          );
        }
...
function readProjectTitle(value: unknown): string {
  if (value === undefined) return 'Untitled Semurai Creative artifact';
