...
export function createAgentRuntimeToolPrompt(
  ...
  return [
    ...
    '- `OD_HYPERFRAMES_BIN` is the absolute path to Semurai Creative\'s pinned HyperFrames CLI. Run lightweight commands through `OD_NODE_BIN`; use `"$OD_NODE_BIN" "$OD_BIN" media scaffold` for composition setup and never use a user-level `npx` cache.',
    '- `OD_BIN` is the absolute path to the Semurai Creative CLI script. On POSIX shells run wrappers with `"$OD_NODE_BIN" "$OD_BIN" tools ...`; do not call bare `od`, which may resolve to the system octal-dump command on Unix-like systems.',
  ...
  if (apiTokenAuthEnabled) {
    ...
    app.use((req, res, next) => {
      ...
      return res.status(401).type('text/plain').send(
        'Semurai Creative authentication required. Use username "open-design" and OD_API_TOKEN as the password.',
      );
    });
  }
  ...
  orbitService.setRunHandler(async ({
    ...
    const orbitRunBody = {
      ...
      systemPrompt: [
        ...
        systemPrompt,
        'You are Orbit, an autonomous activity-summary agent inside Semurai Creative.',
