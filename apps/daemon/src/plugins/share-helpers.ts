...
export const PLUGIN_SHARE_ACTION_LABELS: Record<PluginShareAction, string> = {
  ...
  'contribute-open-design': 'Contribute to Semurai Creative',
};
...
export function renderPluginSharePrompt({ action, sourcePlugin, stagedPath }: PluginSharePromptInput): string {
  ...
  if (action === 'publish-github') {
    return [
      `Publish the local Semurai Creative plugin "${title}" as a new public GitHub repository.`,
      '',
      `The plugin source files have been copied into this project at \`${stagedPath}\`.`,
      'Use the local daemon share endpoint so the publish flow runs through Semurai Creative\'s validated GitHub path:',
      '',
      '```bash',
  ...
  return [
    `Open a pull request to add the local Semurai Creative plugin "${title}" to the Semurai Creative repository.`,
    '',
    `The plugin source files have been copied into this project at \`${stagedPath}\`.`,
    'Use the local daemon share endpoint so the contribution flow runs through Semurai Creative\'s validated GitHub path:',
    '',
    '```bash',
...
export function upsertSkillPluginCandidateAssistantMessage(
  ...
  upsertMessage(db, run.conversationId, {
    id: messageId,
    role: 'assistant',
    content: `Semurai Creative found reusable skill material that can become a plugin: ${candidate.title}`,
    events: [{
