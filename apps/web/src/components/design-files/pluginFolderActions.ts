...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the Semurai Creative community catalog at `nexu-io/open-design`.',
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'Run this deterministic Semurai Creative CLI workflow from the current project workspace:',
    '',
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to open-design`. That command emits a Semurai Creative issue URL and belongs to the "Semurai Creative PR" button.',
    '',
    'Run this deterministic Semurai Creative CLI workflow from the current project workspace:',
    '',
