...
export function buildAgentGuideMarkdown(options: AgentGuideOptions = {}): string {
  ...
  lines.push('# Semurai Creative — agent setup guide');
  lines.push('');
  lines.push(
    'You are a coding agent (Claude Code, Codex, Cursor, openclaw, hermes, or similar). ' +
      'The user wants Semurai Creative wired into your workflow. Semurai Creative is a local ' +
  ...
  if (options.versionHint) {
    lines.push('');
    lines.push(`> Reported Semurai Creative version: \`${options.versionHint}\``);
  }
  lines.push('');
  ...
  lines.push('   ```');
  lines.push('');
  lines.push('   If it 404s or times out, ask the user to run `pnpm tools-dev` (dev) or open the Semurai Creative app (packaged).');
  lines.push('');
  if (installInfo) {
