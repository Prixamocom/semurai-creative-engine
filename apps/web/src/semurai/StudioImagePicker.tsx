...
export function StudioImagePicker({ target, locale, api, onApply, onClose }: {
  ...
  return <Dialog ariaLabel={c.title} className={styles.dialog} onClose={busy ? undefined : onClose} closeOnEscape>
    <div ref={root} onKeyDown={event => {
      ...
      <header className={styles.header}><h2>{c.title}</h2><Button title={c.close} disabled={busy} onClick={onClose}><X size={18} /></Button></header>
      <StudioMedia picker images={images} onChange={value => { setImages(value); setAlt(value[0]?.description || value[0]?.title || ''); }} useLibrary={false} onLibrary={() => {}} api={api} locale={locale} disabled={false} onBusy={setBusy} />
