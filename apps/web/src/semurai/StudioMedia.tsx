...
export function StudioMedia({ images, onChange, useLibrary, onLibrary, api, locale, disabled, onBusy, picker = false, initialSource }: {
  ...
  locale: 'pl' | 'en' | 'de'; disabled: boolean; onBusy: (busy: boolean) => void; picker?: boolean; initialSource?: 'attach' | 'library';
}) {
  ...
  const closeLibrary = () => { setOpen(false); libraryButton.current?.focus(); };
  ...
  return <div ref={root} className={styles.media}>
    <div className={styles.actions}><Button disabled={disabled || busy || images.length >= limit} onClick={() => file.current?.click()} title={c.attach}><Paperclip size={16} />{c.attach}</Button><Button ref={libraryButton} disabled={disabled || busy} onClick={() => { setOpen(true); void search(); }} title={c.library}><Images size={16} />{c.library}</Button></div>
