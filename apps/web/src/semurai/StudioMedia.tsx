...
export function StudioMedia({ images, onChange, useLibrary, onLibrary, api, locale, disabled, onBusy }: {
  ...
  locale: 'pl' | 'en' | 'de'; disabled: boolean; onBusy: (busy: boolean) => void;
}) {
  ...
  async function upload(files: File[]) {
    ...
    await work(async () => {
      ...
      for (const item of files.slice(0, 6 - images.length)) {
  ...
  return <div ref={root} className={styles.media}>
    <div className={styles.actions}><Button disabled={disabled || busy || images.length >= 6} onClick={() => file.current?.click()} title={c.attach}><Paperclip size={16} />{c.attach}</Button><Button disabled={disabled || busy} onClick={() => { setOpen(!open); if (!open) void search(); }} title={c.library}><Images size={16} />{c.library}</Button></div>
    <input hidden ref={file} type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={event => { void upload([...event.target.files ?? []]); event.target.value = ''; }} />
    ...
    <label className={styles.auto}><input type="checkbox" checked={useLibrary} disabled={disabled} onChange={event => onLibrary(event.target.checked)} />{c.auto}</label>
    {open && <section><div className={styles.actions}><input aria-label={c.search} placeholder={c.search} value={query} onChange={event => setQuery(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') { event.preventDefault(); void search(); } }} /><Button disabled={busy} onClick={() => { void search(); }}>{c.search}</Button></div>{!busy && !items.length && <p>{c.empty}</p>}<div className={styles.grid}>{items.map(item => <Button key={item.id} disabled={images.length >= 6 || images.some(value => value.id === item.id)} onClick={() => onChange([...images, item])}><img src={item.dataUrl} alt={item.title} /><span>{item.title}</span></Button>)}</div>{more && <Button disabled={busy} onClick={() => { void search(page + 1); }}>{c.more}</Button>}</section>}
