...
import { withStudioSpeakerNotes } from './studio-pptx-notes';
...
export async function exportStudioDocument(source: string, deck: boolean, kind: 'pptx' | 'print', path: string, title: string, notes: readonly (string | null)[] = []) {
  let scripts: string[] = [];
  if (kind === 'pptx') {
  ...
  return kind === 'pptx' && result
    ? withStudioSpeakerNotes(result, Array.from({ length: studioSlideCount(source) }, (_, index) => notes[index] ?? ''))
    : result;
}
