...
describe('WhatsNewPopup version resolution', () => {
  ...
  it('never paints the placeholder while /api/version is still in flight', async () => {
    ...
    expect(screen.getByText(`Semurai Creative ${SHOW_PAYLOAD.version} is here`)).toBeTruthy();
  });
  ...
  it('switches to the running version once /api/version resolves', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(`Semurai Creative ${RUNNING_APP_VERSION} is here`)).toBeTruthy();
    });
    expect(screen.queryByText(`Semurai Creative ${SHOW_PAYLOAD.version} is here`)).toBeNull();
  });
