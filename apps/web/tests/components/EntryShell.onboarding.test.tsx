...
async function findCloudSignInButton() {
  return screen.findByRole('button', { name: /Sign in to Semurai Creative/i });
}
...
describe('EntryShell onboarding Semurai Creative AMR runtime', () => {
  it('gates Home on an authoritative signed-out Cloud session without clearing saved setup', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: 'Sign in to Semurai Creative' }),
    ).toBeTruthy();
  ...
  it.each([
    ...
    expect(
      screen.queryByRole('heading', { name: 'Sign in to Semurai Creative' }),
    ).toBeNull();
  });
  ...
  it('shows the model-source chooser after Cloud sign-in without exposing legacy onboarding steps', async () => {
    ...
    expect(screen.getByRole('radio', { name: /Semurai Creative Hosted/i })).toBeTruthy();
  ...
  it('supports arrow-key selection and focus within the model-source radio group', async () => {
    ...
    const hosted = await screen.findByRole('radio', { name: /Semurai Creative Hosted/i });
  ...
  it('does not auto-select Semurai Creative AMR when the AMR runtime is unavailable', async () => {
    globalThis.fetch = vi.fn(async () =>
    ...
    expect(await screen.findByRole('heading', { name: 'Sign in to Semurai Creative' })).toBeTruthy();
    expect(await findCloudSignInButton()).toBeTruthy();
    expect(screen.queryByRole('button', { name: /Semurai Creative AMR/i })).toBeNull();
  ...
  it('shows Semurai Creative Cloud as the default connect surface when AMR is available', async () => {
    globalThis.fetch = vi.fn(async () =>
      jsonResponse({ loggedIn: false, profile: 'prod', user: null, configPath: '/x' }),
    ) as typeof fetch;
    renderOnboarding();

    expect(screen.getByRole('heading', { name: 'Sign in to Semurai Creative' })).toBeTruthy();
    expect(await findCloudSignInButton()).toBeTruthy();
  ...
  it('clears AMR login pending when canceled and allows a fresh sign-in attempt', async () => {
    ...
    const cloudButton = await screen.findByRole('button', {
      name: /Sign in to Semurai Creative/i,
    });
  ...
  it('cancels AMR login and re-enables onboarding after the login timeout', async () => {
    ...
    expect(
      screen
        .getByRole('button', { name: /Sign in to Semurai Creative/i })
  ...
  it('continues normally when Semurai Creative AMR is signed in', async () => {
    globalThis.fetch = vi.fn(async () =>
      jsonResponse({
        loggedIn: true,
        profile: 'prod',
  ...
  it('keeps the cloud sign-in CTA visible after detection settles without surfacing AMR', async () => {
    ...
    expect(
      await screen.findByRole('button', { name: /Sign in to Semurai Creative/i }),
    ).toBeTruthy();
    expect(screen.queryByRole('button', { name: /Semurai Creative AMR/i })).toBeNull();
    expect(document.querySelector('.onboarding-view__card--skeleton')).toBeNull();
  });
