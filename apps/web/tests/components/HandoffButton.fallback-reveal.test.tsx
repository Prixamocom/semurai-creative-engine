...
describe('HandoffButton zero-editors fallback', () => {
  ...
  it('copies a framework-specific CLI handoff prompt with the local project path', async () => {
    ...
    expect(screen.queryByRole('link', { name: /打开 Semurai Creative Cloud 官网/ })).toBeNull();
    expect(screen.getByTestId('handoff-cli-item-amr').textContent).toContain('Semurai Creative');
